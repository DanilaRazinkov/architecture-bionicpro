# /opt/airflow/dags/crm_telemetry_dag.py
...
def extract_users(**kwargs):
    ...
    # Сериализуем DataFrame в CSV для XCom
    csv_buffer = StringIO()
...
# --- TASK 2: Extract Telemetry from ClickHouse ---
def extract_and_aggregate_telemetry(**kwargs):
    ...
    if not rows:
        ...
        # Конвертируем список кортежей в DataFrame
    ...
    if telemetry_df.empty:
        ...
        logging.info("⚠️ Нет данных для агрегации — создана пустая таблица.")
    else:
        agg_df = telemetry_df.groupby('user_id').agg({
            'signal_time': ['count', 'max'],  # count → total_signals, max → last_signal_time
        ...
        # Переименовываем колонки после агрегации
        agg_df.columns = [
            'user_id',
            'total_signals',
            'last_signal_time',
    ...
    # Замена NaT на None для сериализации
